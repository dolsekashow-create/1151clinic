...
/**
 * كلمة مرور يضبطها المدير.
 *
 * ⚠️ اختيارية عمدًا: المسار الافتراضي يبقى «رسالة يضبطها الموظف بنفسه» وهو
 *    الأسلم — لا أحد يعرف كلمة المرور غير صاحبها. الضبط المباشر موجود لأن
 *    خادم البريد قد لا يكون مهيّأً، وعندها البديل الوحيد هو تعطيل إنشاء
 *    المستخدمين كليًا.
 * ⚠️ 10 محارف حدٌّ أدنى (أعلى من حدّ Supabase الافتراضي): كلمة المرور هنا
 *    يضعها طرف ثالث وتُنقل شفهيًا، فاحتمال ضعفها أعلى من التي يختارها صاحبها.
 * ⚠️ لا تُخزَّن ولا تُعاد ولا تُسجَّل في أي مكان — تُمرَّر إلى Auth ثم تُنسى.
 */
const initialPasswordSchema = z
  .string()
  .min(10, 'كلمة المرور يجب ألا تقل عن 10 محارف')
  .max(72, 'كلمة المرور طويلة جدًا')
  .refine((v) => /[A-Za-z]/.test(v) && /[0-9]/.test(v), 'يجب أن تحتوي حروفًا وأرقامًا');
...
export const userCreateSchema = z
  .object({
    ...
    /** يضبطها المدير، أو تُترك فارغة فتُرسَل رسالة للموظف. */
    initialPassword: initialPasswordSchema.optional().or(z.literal('')),
    phone: phoneSchema,
