...
> **الحالة:** هذا المستند هو **التصميم المعتمد للـ ERD**. تنفيذ الجداول يبدأ في **Phase 2** (الأمان والهوية) ثم **Phase 3/4**.
> Phase 1 نفّذت فقط الأساس التقني: المخططات (schemas)، الامتدادات (extensions)، ودوال البنية المشتركة.
...
كل جدول في `public` يُفعَّل عليه `ENABLE ROW LEVEL SECURITY` + `FORCE ROW LEVEL SECURITY`، ولا يُترك أي جدول بلا سياسة.

راجع [SECURITY.md](./SECURITY.md) §3 لقوالب السياسات ودوال `app.*`.
...
- `supabase/seed/` للبيانات المرجعية فقط (permissions، roles، وحدات القياس) — **لا بيانات عملاء أو مالية.**
